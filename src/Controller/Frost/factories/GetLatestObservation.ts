...
export class GetLatestObservationConverter implements ResultModelConverter<Observation> {
    public convert(json: ResultList, options: GetLatestObservationOptions): Observation {
        ...
        if (json.value[0].Observations === null || json.value[0].Observations === undefined) {
            throw new Error("nö");
        }
        if (json.value[0].Observations[0] === null || json.value[0].Observations[0] === undefined) {
            throw new Error("nö");
        }

        let observation: ObservationsEntity = json.value[0].Observations[0];

        return new Observation(options.station, options.feature, observation.result, new Date(observation.phenomenonTime));
    }
}
